...
const PlayScenarioPage = () => {
  ...
  const [state, send] = useMachine(machine || createDialogueMachine("empty", []), {
    key: machineKey
  });
  ...
  const handleOptionClick = async (option: DialogueOption) => {
    ...
    setTimeout(() => {
      const currentStep = selectedDialogue?.steps.find(step => 
        step.id === option.next
      );
              ...
              {currentStep?.options && currentStep.options.length > 0 && (
                <>
                  <div className="response-prompt">
                    ...
                    <p>Choose your response carefully - it affects your social skills score!</p>
                  </div>
