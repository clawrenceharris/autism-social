...
export const useGemini = (
  initialOptions?: Partial<UseGeminiOptions>
): UseGeminiReturn => {
  ...
  const getModel = useCallback((): GenerativeModel => {
    ...
    if (!apiKey) {
      throw new Error(
        "Gemini API key is required. Provide it in the options or set VITE_GEMINI_KEY in your environment."
      );
    }

    const genAI = new GoogleGenerativeAI(apiKey);
    return genAI.getGenerativeModel({
      model: optionsRef.current.model || "gemini-1.5-pro",
    });
  }, []);
  ...
  const generateConfig = useCallback(
    (options?: Partial<UseGeminiOptions>): GenerationConfig => {
  ...
  const getCacheKey = useCallback(
    (prompt: string, options?: Partial<UseGeminiOptions>): string => {
      const mergedOptions = { ...optionsRef.current, ...options };
