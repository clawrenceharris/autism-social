...
import type { ScoreSummary, ScoreCategory, DialogueStep, Message } from "../../../types";
...
interface StepAnalysis {
  ...
  userResponse: string;
  npcMessage: string;
  pointsEarned: Record<ScoreCategory, number>;
...
const DialogueCompletionModal: React.FC<DialogueCompletionModalProps> = ({
  ...
  const [loadingBetterResponse, setLoadingBetterResponse] = useState<string | null>(null);
  ...
  const stepAnalyses: StepAnalysis[] = dialogueSteps.map((step, index) => ({
    stepId: step.id,
    userResponse: userMessages[index]?.text || "Sample user response",
    npcMessage: step.npc,
    pointsEarned: {
      clarity: Math.floor(Math.random() * 3),
      empathy: Math.floor(Math.random() * 3),
      assertiveness: Math.floor(Math.random() * 3),
      social_awareness: Math.floor(Math.random() * 3),
      self_advocacy: Math.floor(Math.random() * 3),
    }
  }));
  ...
  const getCategoryIcon = (category: ScoreCategory) => {
    switch (category) {
      case "clarity": return <MessageSquare size={16} />;
      case "empathy": return <Award size={16} />;
      case "assertiveness": return <Target size={16} />;
      case "social_awareness": return <Eye size={16} />;
      case "self_advocacy": return <TrendingUp size={16} />;
  ...
  const totalPointsEarned = Object.values(scores).reduce((sum, score) => sum + score, 0);
  const averageScore = Math.round(totalPointsEarned / categories.length);
  ...
  return (
    <div className="completion-modal">
        ...
        <div className="overall-score">
          ...
          <div className="score-breakdown">
            <div className="total-points">
              <span className="points-earned">+{totalPointsEarned}</span>
      ...
      <div className="analysis-toggle">
        ...
        >
          {analysisMode ? <EyeOff size={20} /> : <Eye size={20} />}
      ...
      <div className="progress-section">
        ...
        <div className="progress-categories">
          {categories.map((category, index) => {
            const currentScore = getPreviousValue(category);
              ...
              <div key={index} className="progress-item">
                <div className="progress-header">
                  ...
                  <div className="score-display">
                    ...
                    <span className={`current-score ${scoreLevel}`}>
                      {currentScore}%
                    </span>
                  </div>
                </div>
                ...
                <div className="progress-bar-container">
                  <div 
                    className={`progress-bar ${scoreLevel}`}
                    style={{ width: `${Math.min(currentScore, 100)}%` }}
                  >
                    <div className="progress-shine"></div>
                  </div>
                  <div className="progress-percentage">{currentScore}%</div>
                </div>
                ...
                <div className="step-header">
                  <div className="step-info">
                    ...
                    <div className="step-points">
                      {Object.entries(analysis.pointsEarned).map(([cat, points]) => 
                        points > 0 && (
                          <span key={cat} className="point-badge">
                            {getCategoryIcon(cat as ScoreCategory)}
                            +{points}
                          </span>
                        )
                      )}
                  ...
                  <div className="step-details">
                    <div className="conversation-flow">
                      ...
                      <div className="message-analysis npc">
                        <div className="message-bubble">
                          <div className="message-text">{analysis.npcMessage}</div>
                        </div>
                        <button 
                          onClick={() => generateBetterResponse(analysis.stepId)}
                          className="analysis-btn better-response"
                          disabled={loadingBetterResponse === analysis.stepId}
                        >
                          <Lightbulb size={16} />
                          {loadingBetterResponse === analysis.stepId ? 'Generating...' : 'Better Response'}
                        </button>
                      </div>
                      ...
                      <div className="message-analysis user">
                        <div className="message-bubble">
                          <div className="message-text">{analysis.userResponse}</div>
                        </div>
                        <button 
                        ...
                        >
                          ...
                          {loadingFeedback === analysis.stepId ? 'Analyzing...' : 'Get Feedback'}
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="points-breakdown">
                      ...
                      <div className="points-grid">
                        {Object.entries(analysis.pointsEarned).map(([category, points]) => (
                          <div key={category} className={`point-item ${points > 0 ? 'earned' : ''}`}>
                            {getCategoryIcon(category as ScoreCategory)}
                            <span className="category">{formatCategoryName(category as ScoreCategory)}</span>
