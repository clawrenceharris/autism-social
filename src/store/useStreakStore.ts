    ...
    (set, get) => ({
      ...
      fetchStreak: async (userId: string) => {
        try {
          ...
          set({ 
            error: err instanceof Error ? err.message : "Failed to load streak data",
            loading: false 
          });
        }
      },
      ...
      incrementStreak: async (userId: string) => {
        try {
          ...
          const timezone = currentData.lastCompletionTimezone || 
          ...
          return {
            ...
            message: streakIncremented 
              ? "Streak increased!" 
              : (streakReset ? "Streak reset and started new streak" : "Streak maintained")
          };
        } catch (err) {
      ...
      resetStreak: async (userId: string) => {
        try {
          ...
          set({ 
            error: err instanceof Error ? err.message : "Failed to reset streak",
            loading: false 
          });
        }
      },
      ...
      checkAndUpdateStreak: async (userId: string) => {
        try {
          ...
          set({ 
            error: err instanceof Error ? err.message : "Failed to check streak",
            loading: false 
          });
